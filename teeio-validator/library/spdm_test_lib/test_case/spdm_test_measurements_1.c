...
bool spdm_test_measurements_1_setup(void *test_context)
{
  ...
  return m_spdm_test_group_measurements[0].case_setup_func(&m_spdm_test_context);
}

bool spdm_test_measurements_1_run(void *test_context)
{
  m_spdm_test_group_measurements[0].case_func(&m_spdm_test_context);
  return true;
}

bool spdm_test_measurements_1_teardown(void *test_context)
{
  m_spdm_test_group_measurements[0].case_teardown_func(&m_spdm_test_context);
  return true;
}
