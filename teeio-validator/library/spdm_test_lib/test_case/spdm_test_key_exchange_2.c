...
bool spdm_test_key_exchange_2_setup(void *test_context)
{
  ...
  return m_spdm_test_group_key_exchange_rsp[1].case_setup_func(&m_spdm_test_context);
}

bool spdm_test_key_exchange_2_run(void *test_context)
{
  m_spdm_test_group_key_exchange_rsp[1].case_func(&m_spdm_test_context);
  return true;
}

bool spdm_test_key_exchange_2_teardown(void *test_context)
{
  m_spdm_test_group_key_exchange_rsp[1].case_teardown_func(&m_spdm_test_context);
  return true;
}
