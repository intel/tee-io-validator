...
bool static gen_default_iv(uint32_t* iv, int iv_size)
{
  if(iv_size != 12) {
    TEEIO_ASSERT(false);
    return false;
                                  ...
                                  )
{
  ...
  bool key_or_iv_capable = key_gen_capable || iv_gen_capable;
  ...
  if(g_teeio_fixed_key) {
    ...
    *cxl_ide_km_iv = CXL_IDE_KM_KEY_IV_DEFAULT;
    ...
    if(key_or_iv_capable && direction == CXL_IDE_KM_KEY_DIRECTION_TX) {
