...
static void test_cxl_ide_key_prog_8 (
  ...
  int case_class, int case_id)
{
  ...
  // If the device is not capable of generating KEY, then use the dynamic KEY
  if(caps.ide_key_generation_capable == 0) {
    cxl_ide_km_aes_256_gcm_key_buffer_t temp_key_buffer = {0};
    cxl_ide_prepare_dynamic_keys(&temp_key_buffer, &cxl_ide_km_iv_tx);
    memcpy(&tx_key_buffer.key, &temp_key_buffer.key, sizeof(tx_key_buffer.key));
  }
...
PrepareDone:
  ...
  // prepare the test IVs
  const char* test_iv_strs[] = {
    "IDE_IV = 0x1",
    "GeneratedIV - 1",
    "GeneratedIV + 1"
  };
  uint32_t test_ivs[3][3] = {0};
  test_ivs[0][2] = 0x1;
  memcpy(&test_ivs[1], &tx_key_buffer.iv, sizeof(tx_key_buffer.iv));
  test_ivs[1][2] -= 1;
  memcpy(&test_ivs[2], &tx_key_buffer.iv, sizeof(tx_key_buffer.iv));
  test_ivs[2][2] += 1;

  int i;
  TEEIO_DEBUG((TEEIO_DEBUG_INFO, "TX Key_Iv:\n"));
  ...
  for(i = 0; i < 3; i++) {
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "Test TX Key_Iv(%d):\n", i));
    memcpy(tx_key_buffer.iv, &test_ivs[i], sizeof(tx_key_buffer.iv));
    dump_key_iv_in_key_prog(tx_key_buffer.key, sizeof(tx_key_buffer.key)/sizeof(uint32_t), tx_key_buffer.iv, sizeof(tx_key_buffer.iv)/sizeof(uint32_t));

    teeio_record_assertion_result(case_class, case_id, 0, IDE_COMMON_TEST_CASE_ASSERTION_TYPE_SEPARATOR, TEEIO_TEST_RESULT_NOT_TESTED, "  port_index = 0x%02x %s", port_index, test_iv_strs[i]);
...
bool cxl_ide_test_key_prog_8_setup(void *test_context)
{
  ...
  bool iv_generation_capable = false;
  for(int i = 0; i <= max_port_index; i++) {
    ...
    if(dev_cap->iv_generation_capable == 0) {
      ...
      iv_generation_capable = true;
    }
  }

  if(iv_generation_capable == false) {
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "CXL IDE IV Generation is not supported in all the ports of the device. Skip the case.\n"));
...
void cxl_ide_test_key_prog_8_run(void *test_context)
{
  ...
  for(int i = 0; i <=lower_port->cxl_data.query_resp.max_port_index; i++) {
    if(m_dev_caps[i].iv_generation_capable == 1) {
      test_cxl_ide_key_prog_8(group_context->spdm_doe.doe_context, group_context->spdm_doe.spdm_context,
