...
bool tdisp_test_interface_report_5_setup (void *test_context)
{
	...
	if (!tdisp_test_set_ide_stream (test_context, PCI_IDE_KM_KEY_SET_K0)) {
		TEEIO_DEBUG ((TEEIO_DEBUG_ERROR,
			"tdisp_test_interface_report_1_setup set_ide_stream failed.\n"));
	...
	if (!tdisp_test_lock_interface (test_context, g_tdisp_interface_id.function_id,
		...
		TEEIO_DEBUG ((TEEIO_DEBUG_ERROR,
			"tdisp_test_interface_report_1_setup lock_interface failed.\n"));
...
void tdisp_test_interface_report_5_run (void *test_context)
{
	...
	uint16_t interface_info = data16.raw;
	READ_DATA32 data32 = {
		.byte0 = interface_report_buffer[12],
		.byte1 = interface_report_buffer[13],
		.byte2 = interface_report_buffer[14],
		.byte3 = interface_report_buffer[15]
	};
	uint32_t mmio_range_count = data32.raw;

	data32.byte0 = interface_report_buffer[16 +
			mmio_range_count * 16];
	data32.byte1 = interface_report_buffer[17 +
			mmio_range_count * 16];
	data32.byte2 = interface_report_buffer[18 +
			mmio_range_count * 16];
	data32.byte3 = interface_report_buffer[19 +
			mmio_range_count * 16];
	uint32_t device_specific_info_len = data32.raw;

	bool res = (buffer_size == (16 + mmio_range_count * 16 + 4 + device_specific_info_len));
	...
	for (uint32_t i = 0; i < mmio_range_count; ++i) {
		...
		data32.byte3 = interface_report_buffer[19 +
				i * 16 + 12];
		uint32_t range_attributes = data32.raw;
